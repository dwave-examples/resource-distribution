...
def run_page():
    ...
    if run_button:
        if validate_input(form):
            ...
            if result:
                ...
                results_dict['Utility'].append(str(round(result.total_utility, 2)))
                results_dict['Cost'].append(str(round(result.total_cost, 2)))
                results_dict['Energy'].append(str(round(result.energy, 2)))
                results_dict['Run Time'].append(str(round(result.t, 2)))
                folium_map = figure
            else:
    ...
    if results_dict:
        try:
            ...
            results_dict.clear()
            st.error("Something went wrong. Refresh the page and try again please.")
            print(e)
